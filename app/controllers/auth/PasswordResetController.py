...
class PasswordResetController(Controller):
    def show(self, view: View) -> View:  # Show password_reset page
        return view.render("auth.password_reset")

    def store(self, auth: Auth, request: Request, response: Response) -> Response:
    ...
    def change_password(self, view: View, request: Request) -> View:
        return view.render("auth.change_password", {"token": request.param("token")})

    def store_changed_password(self, auth: Auth, request: Request, response: Response) -> Response:
